...
stringly=$(printf '%s' "$new_code" | grep -nE 'match [A-Za-z_][A-Za-z0-9_.]*\.(as_str|as_deref)\(\)|matches!\([A-Za-z_][A-Za-z0-9_.]*\.(as_str|as_deref)\(\)|\.as_str\(\) *[!=]= *"|\.as_deref\(\) *[!=]= *Some\("|let Some\("|contains\(&[A-Za-z_][A-Za-z0-9_.]*\.as_str\(\)\)|map\(\|[a-z_]+\| *[a-z_]+ *[!=]= *"' | head -5 || true)
